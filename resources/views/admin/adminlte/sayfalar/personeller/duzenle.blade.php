...
@section('content')
    @section('title', config("const.personeller").' - '.config("const.duzenle").' - '.config("const.yonetim_paneli"))
    <!-- Content Header (Page header) -->
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>{{config("const.personel")}} {{config("const.duzenle")}}</h1>
                </div>
                <div class="col-sm-6">
                    ...
                    <div class="card card-warning">
                        <div class="card-header">
                            <h3 class="card-title"><i
                                    class="fas fa-user-plus"></i> #{{$get_personel->id}} {{config("const.personel")}} {{config("const.duzenle")}}
                            </h3>
                        </div>
                        <!-- /.card-header -->
                        <!-- form start -->
                        ...
                        <form id="yeniPersonelGrubuEkle" method="post"  enctype="multipart/form-data"
                              action="{{route("admin.personeller_duzenle.post", ["id" => $get_personel->id])}}">
                            @csrf
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" name="ad" class="form-control" id="forName"
                                               placeholder="Ad" value="{{old("ad", $get_personel->ad)}}">
                                    </div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" name="soyad" class="form-control" id="forEmail"
                                               placeholder="Soyadı" value="{{old("soyad", $get_personel->soyad)}}">
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label>Grup</label>
                                    ...
                                    <select class="form-control select2"  name="grup">
                                        @foreach($personel_gruplari as $personel_grubu)
                                            <option @if(old("grup", $get_personel->personel_grubu_id) == $personel_grubu->id) selected @endif value="{{$personel_grubu->id}}">{{$personel_grubu->grup_adi}}</option>
                                        @endforeach
                                    </select>
                                    </div>
                                </div>
